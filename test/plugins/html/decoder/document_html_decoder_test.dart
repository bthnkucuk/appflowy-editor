...
void main() async {
  group('document_html_decoder_test.dart', () {
    ...
    test('parse html sample 1', () async {
      ...
      expect(result.toJson(includeId: false, includeDatabaseIndex: false, includeRank: false), documentSample1);
    });

    test('parse html sample 2', () async {
      // sample 2 contains table
      final result = DocumentHTMLDecoder().convert(htmlSample2);
      expect(result.toJson(includeId: false, includeDatabaseIndex: false, includeRank: false), documentSample2);
    });

    test('parse html sample 3', () async {
      // sample 3 contains lists
      final result = DocumentHTMLDecoder().convert(htmlSample3);
      expect(result.toJson(includeId: false, includeDatabaseIndex: false, includeRank: false), documentSample3);
    });

    test('parse html sample 4', () async {
      // sample 4 contains lists
      final result = DocumentHTMLDecoder().convert(htmlSample4);
      expect(result.toJson(includeId: false, includeDatabaseIndex: false, includeRank: false), documentSample4);
    });
  });
}
