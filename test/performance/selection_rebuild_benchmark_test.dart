...
void main() {
  group('Selection rebuild cost', () {
    testWidgets('extend collapsed selection 100× within one paragraph', (tester) async {
      // ~80-char paragraph: realistic single-line selection growth.
      const text = 'The quick brown fox jumps over the lazy dog while a curious cat watches nearby.';
      ...
      // Warm up.
      for (var i = 1; i <= 10; i++) {
        editorState.selection = Selection.single(path: [0], startOffset: 0, endOffset: i);
        await tester.pump();
      }
      ...
      for (var i = 1; i <= iters; i++) {
        editorState.selection = Selection.single(path: [0], startOffset: 0, endOffset: (i % (text.length - 1)) + 1);
        await tester.pump();
      }
      sw.stop();
    ...
    testWidgets('extend selection 50× across 5 paragraphs', (tester) async {
      ...
      final editor = tester.editor..addParagraphs(5, initialText: 'A paragraph of decent length here.');
      await editor.startTesting();
      final editorState = editor.editorState;
    ...
    testWidgets('duplicate selection set 100× (no actual change — should short-circuit)', (tester) async {
      final editor = tester.editor..addParagraph(initialText: 'Sample text here');
      await editor.startTesting();
      final editorState = editor.editorState;

      final fixedSelection = Selection.single(path: [0], startOffset: 2, endOffset: 10);
